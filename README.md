...
- Every request for lock or release of lock will be sent using the `SendRequest` function and the type of message sent will be specified as an argument

- Every request for lock will warrant a release for lock, hence "RELEASE" will be sent after a timeout starting from the time "OK_ENTER" is received
  - If "WAIT" is received instead of "OK_ENTER", client will start the `listenForServerMessages` function to wait for their turn to enter critical section
